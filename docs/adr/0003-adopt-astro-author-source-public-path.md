...
The important correction from the spike is that the target is not "one authored
Markdown file with every durable and generated field in frontmatter." The normal
writing flow starts with title and body. Recall metadata should be generated by
the content projection workflow, kept out of author source, and become public
metadata only when committed in projection frontmatter or committed in another
...
- Generated state: separate run records for prompt versions, provider output,
  confidence, validation failures, and rejected candidates. This state is not
  public metadata and does not affect public pages directly.
- Build projection: a regenerable Astro input layer that combines author source
  with deterministic or LLM-assisted generated metadata into the shape Astro
  pages and content collections need. Commit/deploy is the acceptance boundary
  for generated metadata written into this layer.
- Generated public data artifacts: optional cross-post indexes or feeds such as
  tag indexes, archive indexes, related-navigation indexes, RSS, sitemap, and
  compact search indexes. These artifacts are allowed when a public surface needs
  them, but they must be generated by documented workflow steps rather than
  hand-authored.
- Generated metadata validation: fields outside the documented contract are
  rejected by schema validation. If a public revision affordance is useful later,
  prefer a link to the canonical GitHub file history for `posts/<slug>.md` over
  adding revision metadata.

Frontmatter may be used in the Astro projection, but it must not become the
primary storage location for author-written source or temporary generated state.
Projection files under `src/content/posts/` are do-not-edit generated artifacts;
normal edits must start from author source or documented generation workflows.
...
- Published post detail pages, home pages, tag pages, RSS, sitemap,
  deterministic related posts, and deterministic OGP outputs should be
...
- Recall metadata is generated into `src/content/posts/ja/<slug>.md`
  frontmatter by the content projection workflow when present.
- Cross-post public data may be generated into additional artifacts when recall
  surfaces need indexes or feeds.
- Generated public data must stay within documented contracts; repository
  history remains available through GitHub when revision inspection is needed.
- `pnpm run project:content -- --check` verifies that
  `src/content/posts/ja/<slug>.md` is the current Astro build projection.
- Commit/build gates should run `pnpm run project:content -- --check` so stale
